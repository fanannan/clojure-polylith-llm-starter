(ns polyguard.hooks
 ...
 file-level 解析が必要な項目（interface 契約・1 ファイル 1 ns）は
...
(defn- reg!
 ...
 位置引数 3 個（node + opts マップ）で （上限 3 引数）と整合。"
 [node opts]
 (api/reg-finding!
 (merge
...
(defn- keyword-value
 "node が keyword 値を持つなら抽出する（型駆動/違反を避ける実装）。
...
;; ---------------------------------------------------------------------------
;;: 関数行数上限（ロジック関数 20 行 / インターフェース関数 50 行）
;;
;; 判定: defn の body 全体が何行に渡るか。単純化のため;; interface.clj 内の defn は 50 行、それ以外は 20 行を上限とする。
;; （警告レベル、error にしない。正当な長さの関数もあるため）
;; ---------------------------------------------------------------------------

(defn- ctx-filename
 "hook ctx または node meta から filename を取り出す（clj-kondo 版差異吸収）。
 clj-kondo バージョンによって ctx に :filename が入る場合と入らない場合があるため 両方を試して nil なら空文字列を返す。"
 [ctx]
 (or (:filename ctx)
...
(defn- count-positional-args
 ...
 destructuring と & args は数えない（の対象外）。

 修正履歴: 当初 (remove api/reg-finding! ...) を使っていたが api/reg-finding! は述語ではなく副作用関数（finding 登録）で node を引数に呼ぶと例外または不正な finding 登録を起こす重大バグだった。
...
;; ---------------------------------------------------------------------------
;;: ドメイン component の top-level atom 禁止
;;
...
;; ---------------------------------------------------------------------------
;;: catch の検査（広範囲 catch / 空 catch）
;;
;; 判定:
;; - (catch Throwable ...) / (catch Exception ...) / (catch java.lang.Throwable ...) /
;; (catch java.lang.Exception ...) → 警告
;; - catch 本体が空 or nil 単一 → 警告
;; ---------------------------------------------------------------------------
...
;; scripts/check-interface-contracts.sh が と併せて担う。本 hook は
