...
**§1.2.5 失敗早期検知との整合**: 本規律は静的機械化ではなく規律レベルの介入だが、read-site が局所化されると将来の局所 clj-kondo hook の対象が明確になる（規約成熟後に再検討）。記述系を重くする `defrecord` 採用や独自マクロ導入より先に、この規律で形を作る順序を取る。
...
- **新規 brick 作成前**: 既存 brick で対応できないか必ず検討し、候補 group が分かる場合は同一 group の既存 brick も確認する
∵ POLYLITH_GUIDE.md §1.2
