...
| 種別 | 責務 | §1.1 原則のどれを担うか |
|---|---|---|
| **component** | 再利用可能な機能ブロック。`interface.clj` で公開、`core.clj` で実装 | ドメイン系は **不変性 + 全域性**、I/O 系は **副作用隔離** |
| **base** | アプリの「外側」。HTTP / CLI / Lambda 等のエントリ。components を束ねる | **副作用隔離**（副作用を物理的に集約） |
