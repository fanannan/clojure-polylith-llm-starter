...
**保守時のチェック**: 新原則を追加したら、以下の遡及適用プロセスを必ず実行する：
  ...
  4. 点検結果と修正内容を §7 の規則に従って 議論アーカイブに記録
...
**機械化可能な規律は `check-workspace-integrity.sh` へ配線する**（原則 2 の具体化）。規約が「ファイル構造・設定・命名・所有権・生成物 drift」のように機械検査できる種類なら、文書に書くだけで終えず、対応する検査を `.llm/scripts/` に実装し `check-workspace-integrity.sh` の集約に配線する。文書の規律と配線済み gate がずれている状態は、規約が人間 / LLM の注意力に依存していることを意味し、`CLAUDE.md` §1.2.1 機械化に反する。配線できない規律（価値判断を要するもの等）は、その旨と代替手段（自己停止プロトコル / レビュー観点）を文書に明示する。新しい機械検査を足したら、その内訳を scripts README にも反映する。
...
2. 独立したプロジェクト用途構成として必要なら以下を実施：
   ...
   - **議論アーカイブに記録**（`.llm/memory/archive/maintainer-discussions/`）。検討した却下案も記録
3. §7 の規則に従い、議論アーカイブへ記録（「議論の軌跡を残す」）
...
2. 議論アーカイブに記録（`.llm/memory/archive/maintainer-discussions/`）
...
4. 議論アーカイブへの記録は**大規模採用時のみ**（軽微な追加は `.clj-kondo/config.edn` のコメントで記録）
...
- **補完的 lint**: Eastwood（マクロ展開解析）は保留候補、採用時は議論アーカイブに記録
...
命名規律として、検討段階の仮称や release 名を配布 artifact の正式名にしない。この仕組みの正式名は Structural Evidence View、生成される人間向け view は Review Fatigue Packet、機械可読 record の schema は `structural-evidence.N` 系とする。
...
5. **点検結果と修正内容を §7 の規則に従って 議論アーカイブに記録**
...
| **非推奨化と削除** | 古くなった記述 | 削除時は 議論アーカイブに経緯を残す |
