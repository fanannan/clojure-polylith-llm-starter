...
具体的には、**Clojure 1.12 + tools.deps + Polylith + Malli + clj-kondo + cljfmt** を必須層とし、用途ごとに必要なライブラリを `STACK_GUIDE.md` §3 機能別節から選んで brick の `deps.edn` に反映する構成のプロジェクトを、**LLM が最も誤りにくい形で立ち上げる**ための初期配布物である。Integrant は Web API・バッチ・ワーカ等の I/O を扱う構成で採用されるが、必須層ではない。ライブラリ配布だけなら採用しない。
...
2. **技術選定として固定されているが、目的依存か？** → Yes なら **推奨カタログ（文書配布）**（どの機能カテゴリで使うかは STACK_GUIDE.md §3 機能別節で判断）
...
- **二重管理の回避**: ワークスペースルートの deps.edn には必須層のみ配置し、用途依存ライブラリは brick に集約
...
| ファイル | 区分 | 備考 |
|---|---|---|
| deps.edn | D | 必須層のみ（正本は STACK_GUIDE.md §2.1）。用途依存ライブラリは brick の deps.edn に配置（選択肢 H、§2.2 参照）。`:dev :extra-deps` には開発支援ライブラリを配置 |
...
原則 13（§4）の 4 区分のうち 3 つに加え、保守議論アーカイブがここに集約される：
...
3. **推奨カタログに入れる場合、どの機能カテゴリに置くか**（STACK_GUIDE.md §3 機能別節で判断）
...
| **.llm/guide/BOOTSTRAP_GUIDE.md** | プロジェクト初期化手順。完了後も移動しない。原則 1 の効率性・原則 11 の実装 |
