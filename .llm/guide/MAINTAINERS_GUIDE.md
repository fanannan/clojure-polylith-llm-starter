...
2. **メジャーバージョンアップ**:
   ...
   - 推奨カタログ内の依存は、該当用途構成の動作検証（§5.6.2）時にまとめて STACK_GUIDE.md §3 機能別節に反映
3. **API 破壊的変更を含む更新**:
...
6. 各用途構成に対応する §3 機能別節の採用時の確認事項を実際に点検
...
2. 配布前に、§5.6.2 の動作検証を該当用途構成について実行
...
4. 検証用プロジェクトで該当用途構成を採用し、brick deps.edn 反映→ `clj -Spath` 確認（§5.6.2）
...
2. 独立した機能領域として必要なら：
   ...
   - 該当する既存用途構成の **§3 機能別節の `;; lib-catalog` block**に反映
...
| **.llm/guide/STACK_GUIDE.md** | 技術選定の一次情報源。機能領域ごとの選定根拠、用途構成、禁止・非推奨ライブラリを保持。本文書 §5.9 で保守規律を規定 |
...
**機械的検索の活用**: 遡及適用時は**推測による影響範囲判断を避け、`grep -rn` 等で該当箇所を網羅確認**する。実例として用途構成の整理時、既存文書の「Integrant」「コメントアウト」「§6.5」等の言及箇所を grep で確認し、影響範囲（9 ファイル・58+ 箇所）を正確に把握できた。LLM は「意識的に機械的確認を行う」規律を持たないと、知識の穴を推測で埋める傾向がある。
...
本テンプレートの原則（疲労最小化、三基底原則、13 原則）は、**テンプレート自身の設計にも適用される**。テンプレート保守者が新規の規約・用途構成・手続き・網羅を追加しようとする時、その追加自体が原則に反していないかを必ず点検する。

#### 9.4.1 具体的な陥穽

以下の動きは原則違反の典型：

- **網羅の追求**: 用途構成表、機能領域、手続きリスト等を「すべて書き尽くそう」とする動き。網羅は永久に達成不可能（疲労最小化と自己矛盾）。**メモリーは「知っていることを記録する」、網羅は「全てを記録しようとする」。性格が全く異なる**
- **形式的な当てはめ**: 判断基準を第一原理に求めず、「用途構成表にあるか」「手順書にあるか」で判断する動き。LLM が未記載領域で思考停止する
- **分野のせいにする**: 「ML は Clojure に向かない」「ゲームは JVM に向かない」等の分野スコープ制限で問題を転換する動き。本テンプレートの採用理由は分野とは独立（LLM の自律的低負担実装）
- **手続きの肥大化**: 「未知への対処フロー」「Q の雛形」等を事前に網羅する動き。網羅対象が用途構成から手続きに変わっただけで、同じ誤り
