...
|---|---|
| **対象** | 重要な設計判断の理由 |
| **更新するタイミング** | 将来「なぜそう決めたか」を辿る必要がある時 |
| **更新しないもの** | 未決事項、継続更新される運用知識、現在仕様 |
| **最初に見る節** | 「ADR を発行すべき基準」 |
...
| 重要な設計判断を採用した | 出す |
| 大きな技術選定変更をした | 出す |
| Q が解決済みになり、決定理由を将来参照したい | 出す |
| 継続参照する現在の事実だけを残したい | 出さない。`KNOWLEDGE.md` |
| まだ未決で議論中 | 出さない。`QUESTIONS.md` |
...
> **呼び出し元**: 本ディレクトリの ADR 群は `CLAUDE.md §8.0`（実装着手前の確認）の「過去の決定の確認」ステップから参照される決定履歴。空の場合の扱い（着手前確認は通過とみなす）は呼び出し元 §8.0 で一括規定する。
>
...
| **QUESTIONS.md** | 未決の判断 | 未対応(open) → 解決済み(resolved) で閉じる |
...
6. **KNOWLEDGE.md の方針・採否・逸脱理由を伴う変更**（変更経緯の記録のため）
7. **Q が解決済み(resolved) になり、その決定経緯が将来参照されうる時**
...
- フォーマット・lint 修正
- バグ修正（通常のコミットログで十分）
- 単発の判断で、将来参照されない事項（Q が解決済み(resolved) → アーカイブで十分）
...
3. Q 解決済み(resolved) 時の反映先判定で「ADR 発行」が選ばれた場合（QUESTIONS.md §0.4）
