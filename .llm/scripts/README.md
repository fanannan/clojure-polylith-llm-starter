...
| `.llm/template-only/tests/check-instrument-cases-smoke.sh` | Instruction-Following Instrument の case catalog を EDN として検査し、non-exploratory case の incident / authored `[mandate:]` trace と unknown incident / mandate / malformed annotation 検出を確認する。LLM は呼ばない。通常ゲート外 |
...
`session-briefing.sh` は `Control Plane` として mode source / operating intent / decision log / next action surface / completion gate を短く表示し、Evidence Plane として active view / human declaration / closed record / `what-now` / residual pending を表示する。`--audit` は通常 briefing の末尾に Briefing Audit を追加し、`--audit --format edn` は audit EDN だけを返す。generated view は生成して終わりではなく、次セッションの読み始めに必ず fingerprint と declaration attachment を確認する。
