...
| `evidence.sh` | Structural Evidence workflow の主入口。`status` / `search` / `predict` / `declare` / `run` / `close` で session 開始、過去 record 検索、着手前、residual 宣言、command-backed evidence 記録、close 直前の evidence state を扱う |
