  ...
  "Generator for .llm/data/libs.edn and related pattern files.
   ...
   entry maps (see entry-schema). The generator:
     ...
     4. emits artifacts consumed by downstream tools:
        ...
        - .llm/data/conflicts.patterns          (coord-a|coord-b|reason pairs)
        - .llm/data/lib-catalog.manifest.edn    (sidecar derivation manifest)
...
(def generator-path ".llm/scripts/gen_lib_catalog.clj")

(def lib-catalog-outputs
  [".llm/data/libs.edn"
   ".llm/data/deprecated-libs.patterns"
   ".llm/data/forbidden-requires.patterns"
   ".llm/data/conflicts.patterns"])
...
(defn- lib-catalog-manifest [source]
  (assoc
   (derivation/make-manifest
    {:id :lib-catalog
     :tool "gen-lib-catalog"
     :output-path ".llm/data/lib-catalog"
     :generator-path generator-path
     :input-paths [source]
     :input-policy {:untracked :error}
     :generated-at "deterministic"
     :regenerate-command "clj -X:gen-lib-catalog"})
   :derivation/outputs lib-catalog-outputs))

(defn- emit-lib-catalog-manifest! [out-file source]
  (spit out-file
        (str ";; GENERATED by .llm/scripts/gen_lib_catalog.clj — do not edit by hand.\n"
             ";; Source : .llm/guide/STACK_GUIDE.md lib-catalog blocks\n"
             ";; Re-run : clj -X:gen-lib-catalog\n"
             "\n"
             (with-out-str
               (pprint/pprint
                (derivation/with-manifest
                 {:schema/version "lib-catalog-manifest.1"
                  :kind :lib-catalog-manifest
                  :artifact/regime :derived-view
                  :outputs lib-catalog-outputs}
                 (lib-catalog-manifest source)))))))
...
(defn generate
  ...
  (let [source  (or (:source opts) ".llm/guide/STACK_GUIDE.md")
    ...
    (emit-lib-catalog-manifest! (io/file dir "lib-catalog.manifest.edn") source)
    (println (format "gen-lib-catalog: %d entries → %s/{libs.edn,deprecated-libs.patterns,forbidden-requires.patterns,conflicts.patterns,lib-catalog.manifest.edn}"
                     (count entries)
                     (.getPath dir)))
