...
clj -Sdeps '{:paths [".llm/scripts"]}' -e '
(require (quote check-repo-context-consistency))

(def strict-caps #{:deps-edn :clj-kondo :cljfmt :malli :polylith :llm-guides})

(defn result [manifest]
  (check-repo-context-consistency/validate manifest))

(defn assert-case [label pred]
  (when-not pred
