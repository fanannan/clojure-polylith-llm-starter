...
意思決定は、次の「権限階層」で判定する。本節は日常作業で見落としやすい高リスク項目の**抜粋だけ**を置き、完全表にない項目や迷うケースは本節で判断を完結させず正本に戻る。詳細な項目別マッピングと 4 種文書の編集権限は COLLABORATION_GUIDE が正本。判断権限（誰が決めるか）と実行主体（誰が実行するか）は別軸である。
∵ .llm/guide/COLLABORATION_GUIDE.md §2
...
本節は REPL を **どう** 使うか（HOW）の規則である。REPL eval が **必要かどうか**（WHEN）は §8.0.0 の検証粒度判定表と「REPL eval 必須トリガ」が決める。本節は WHEN を再定義しない。両者を混同しない。

**§8.0.0 の REPL eval 必須トリガに該当する検証を行うとき、stateless script ではなく `./.llm/scripts/repl-eval.sh` 経由で nREPL 上で行う**（nREPL が起動できる状態にある限り）。これは §1.2.2 ループ短縮の中核実装であり、**LLM が能動的に閉じやすい代替手段（stateless スクリプト）を構造的に塞ぐための規則**。

REPL eval が不要な作業（typo・コメント・文書修正、純粋関数やテストだけの編集など、§8.0.0 が REPL 非必須と判定するもの）には、本節は REPL を義務化しない。「使うべき時は確実に REPL を使い、不要な時は使わない」を機械的に運用可能にするのが本節の目的である。

**禁止される代替手段**（REPL eval 必須トリガに該当する検証を行う場合）:
...
nREPL 未起動なら 1 度だけ起動依頼（§9.0）。起動待ちの間は、REPL を必要としない作業へ進んでよい。REPL eval 必須トリガに該当する検証を stateless script で代替することは、「念のため」「軽い確認だから」を含めて本規則違反。

LLM がこの規則を見落としやすいのは「stateless の方が安全」という偏向（道具選択バイアス）が働くため。本規則は、必須トリガ該当時に限り肯定形（REPL を使え）と否定形（スクリプトを書くな）を併記して、その偏向を構造的に閉じる。
