...
**対処**: ほぼ全て `reduce` か `into` で書ける（§4.2 参照）。
...
### 1.9 `comment` フォームの中のコード忘れ

**兆候**: 実装を `(comment ...)` に書いたまま、本体に戻し忘れる。

**対処**: コミット前に `grep -n "^  (comment" src/` で確認する習慣。Rich comment はテストケース記述用であり、実装は置かない。
...
**library stack 採用時**: Integrant は含めない（ライブラリがライフサイクル管理を強制しない作法）。ライフサイクルはライブラリ利用者に委ねる。

**Integrant を採用しない stack 採用時**: ライフサイクル管理が必要なら、`-main` 内での直列起動・`with-open` 等で明示的にリソース管理する。
